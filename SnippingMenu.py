...
class Menu(QMainWindow):
    ...
    def __init__(self, numpy_image=None, snip_number=None, start_position=(300, 300, 350, 250), anki_image_data=""):
        ...
        self.anki_image_data = anki_image_data

        self.anki_settings_window = AnkiSettingsWindow.AnkiSettingsWindow(self)
        ...
        # Anki Settings
        anki_settings = QAction('Settings', self)
        anki_settings.setIcon(QIcon("snipping-tool\\icons\\settings_icon.jpg"))
        # anki_action.setShortcut('Ctrl+A')
        anki_settings.setStatusTip('Setting for Anki')
        anki_settings.triggered.connect(self.setAnkiSettings)
        ...
        self.toolbar.addAction(anki_settings)
    ...
    def sendToAnki(self):
        ...
        deck_name, model_name, field_name = self.anki_settings_window.GetCardSettings()
        now = datetime.now()
        filename = "screengrabtool_{}.png".format(now.strftime("%d_%m_%Y-%H_%M"))
